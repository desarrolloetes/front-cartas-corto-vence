...
const AddEditCartaListRow = ({
  ...
  return (
    <TableRow
      hover
      //onClick={event => onRowClick(event, row.codigoProducto)}
    ...
    >
      ...
      <TableCell> {row.prodAprobacionParcial === 0 ? 'Aprobada' : 'Rechazada'} </TableCell>
      <TableCell>{row.prodMotivoRechazoParcial}</TableCell>
    </TableRow>
  );
};
